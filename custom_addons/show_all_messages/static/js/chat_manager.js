...
function make_message (data) {
    ...
    Object.defineProperties(msg, {
        ...
        is_all_message: property_descr("all_message")
    });

    if ((_.contains(data.needaction_partner_ids, session.partner_id)) && (data.needaction==true)) {
    ...
    if ((_.contains(data.needaction_partner_ids, session.partner_id)) || (_.contains(data.starred_partner_ids, session.partner_id))) {
        msg.is_all_message = true;
    }
    if (msg.model === 'mail.channel') {
        var real_channels = _.without(msg.channel_ids, 'channel_inbox', 'channel_starred', 'all_message');
...
function fetch_from_channel (channel, options) {
    ...
    var domain =
        ...
        (channel.id === "all_message") ? [['allmessage', '=', true]] :
