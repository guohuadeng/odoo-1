...
class GoodsClassification(models.Model):
    ...
    goods_tariff_id = fields.Many2one(comodel_name="cus_args.goods_tariff", string="cus goods Code TS", required=False, )  # 海关税则编码 / 商品编号 Code_ts 即 hs_code
    ...
    first_unit_id = fields.Many2one(comodel_name="cus_args.unit", string="First Unit", related='goods_tariff_id.first_unit_id',store=False )  # 第一计量单位
    second_unit_id = fields.Many2one(comodel_name="cus_args.unit", string="second Unit", related='goods_tariff_id.second_unit_id',store=False )  # 第二计量单位
    supervision_condition = fields.Char(string="supervision condition", related='goods_tariff_id.supervision_condition',store=False)  # 监管标识/监管标识

    goods_tariff_hs_code = fields.Char(string="cus goods number", related='goods_tariff_id.code_ts', store=True,required=False, )# 税则库商品编号，冗余字段 ，主要是为了在报关单添加商品界面，实现按商品编号搜索归类库
    ...
    #             classify_status = self.env['cus_center.cus_goods_list'].search([('customs_declaration_id', '=', goods_cls_list.customs_declaration_id.id)]).update({'classify_status': 'yes'})
    ...
    def already_reviewed_btn(self):
        ...
        for goods_cls_list in self:
            if goods_cls_list.customs_declaration_id:
                classify_status = self.env['cus_center.cus_goods_list'].search(
