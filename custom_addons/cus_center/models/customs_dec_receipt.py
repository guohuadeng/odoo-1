...
class DecResult(models.Model):
    """ 报关单回执 """
    _name = 'cus_center.dec_result'
    _rec_name = 'seq_No'
    _inherit = ['mail.thread', 'ir.needaction_mixin']
    _description = 'Custom Declaration Result'

    seq_No = fields.Char(string="SeqNo", required=False, size=18, index=True)                  # 数据中心统一编号
    status_id = fields.Many2one(comodel_name="cus_center.dec_res_status", string="Status", required=False, ) # 回执状态
    ...
    customs_dec_id = fields.Many2one(comodel_name="cus_center.customs_dec",
