...
PARSE_CUS_TO_WLY_PATH = config.options.get('parse_cus_to_wly_path',
                                           '/mnt/xml_data/about_wly_xml_data/pre_ex_client/cus_to_wly')
PARSE_CUS_TO_WLY_ATTACH_PATH = config.options.get('parse_cus_to_wly_attach_path',
                                                  '/mnt/xml_data/about_wly_xml_data/pre_ex_client/cus_to_wly_attach_send')
PARSE_SEND_ERROR_XML_PATH = config.options.get('parse_send_error_xml_path',
                                               '/mnt/xml_data/about_wly_xml_data/pre_ex_client/send_error_xml_message')
GENERATE_REC_WLY_TO_XG_PATH = config.options.get('generate_rec_wly_to_cus_path',
                                                 '/mnt/xml_data/about_wly_xml_data/pre_ex_client/rec_wly_to_cus')
GENERATE_REC_WLY_TO_XG_ATTACH_PATH = config.options.get('generate_rec_wly_to_cus_attach_path',
                                                        '/mnt/xml_data/about_wly_xml_data/pre_ex_client/rec_wly_to_cus_attach')
BACKUP_SEND_XML_PATH = config.options.get('backup_send_xml_path',
                                          '/mnt/xml_data/about_wly_xml_data/pre_ex_client/send_backup_xml')  # 新光原始报文备份目录
BACKUP_SEND_ATTACH_XML_PATH = config.options.get('backup_attach_send_xml_path',
                                                 '/mnt/xml_data/about_wly_xml_data/pre_ex_client/send_backup_xml_attach')  # 新光原始报文备份目录

# post_ex_client 后置交换客户端路径
RECV_XML_BASE_PATH = config.options.get('parse_rec_ex_to_wly',
                                        '/mnt/xml_data/about_wly_xml_data/post_ex_client/rec_ex_to_wly')
RECV_XML_ATTACH_BASE_PATH = config.options.get('parse_rec_ex_to_wly_attach',
                                               '/mnt/xml_data/about_wly_xml_data/post_ex_client/rec_ex_to_wly_attach')
ERROR_XML_BASE_PATH = config.options.get('parse_rec_error_xml_path',
                                         '/mnt/xml_data/about_wly_xml_data/post_ex_client/error_xml_message')
BAKUP_XML_BASE_PATH = config.options.get('backup_rec_xml_path',
                                         '/mnt/xml_data/about_wly_xml_data/post_ex_client/backup_rec_xml')
...
class CustomsDeclaration(models.Model):
    ...
    packing_id = fields.Many2one(comodel_name="cus_args.wrap_type", string="Package Type")  # 包装种类
    ...
    #     for obj in self:
    #         query = [('res_model', '=', 'cus_center.customs_dec'), ('res_id', '=', obj.id)]  # 根据res_model和res_id查询附件
    ...
    #     """附件上传 计算附件数量"""
    #     attachment_data = self.env['ir.attachment'].read_group([('res_model', '=', 'cus_center.customs_dec'), ('res_id', 'in', self.ids)], ['res_id'], ['res_id'])
    ...
    def action_get_dec_attachment_view(self):
        """附件上传动作视图"""
        self.ensure_one()
        res = self.env['ir.actions.act_window'].for_xml_id('cus_center', 'dec_action_attachment')
        res['domain'] = [('res_model', '=', 'cus_center.customs_dec'), ('res_id', 'in', self.ids)]
        res['context'] = {'default_res_model': 'cus_center.customs_dec', 'default_res_id': self.id}
        return res
    ...
    def duplicate_current_all_data(self):
        ...
        if customs_declaration_obj_copy:
            ...
            # customs_declaration_obj = self.env['cus_center.customs_dec'].browse(ref_id).copy()
            #
            # cus_goods_list_ids_list = []
            # for line in self:
            #     if line.dec_goods_list_ids:
            #         cus_goods_list_ids_list = [goods.copy().id for goods in line.dec_goods_list_ids]
            # customs_declaration_obj.dec_goods_list_ids |= self.env['cus_center.cus_goods_list'].search([('id', 'in', cus_goods_list_ids_list)])
            #
            # return {
                        ...
                        if values_dic:
                            ...
                            for k, values in values_dic.items():
                                if k == 'CodeTS':
                                    ...
                                    currency_id = self.env['basedata.cus_currency'].search(
                                        [('Code', '=', currency_code)])
                                    ...
                                    first_unit_id = self.env['basedata.cus_unit'].search(
                                    ...
                                    second_unit_id = self.env['basedata.cus_unit'].search(
                                        [('Code', '=', second_unit_code)])
                                    dec_goods_list['second_unit'] = second_unit_id[0].id if len(
                            ...
                            try:
                                ...
                                cus_goods_list_obj = self.env['cus_center.cus_goods_list'].create(dec_goods_list)
    ...
    #                 information_attachment_ids = self.env['ir.attachment'].search([('res_model', '=', 'cus_center.customs_dec'),('res_id', '=', obj.id)])  # 取得附件list
    ...
    #                                     new_attachment = self.env['ir.attachment'].search([('res_model', '=', 'cus_center.customs_dec'),('res_id', '=',obj.id),('name', '=',attach_name)]).update({'datas': binary_data})
    #
    ...
    def print_check_reports(self):
        """"""
        return self.env['report'].get_action(self, 'cus_center.report_dec_customs_check_template')

    @api.multi
    def print_invoice(self):
        """打印发票按钮"""
        self.env['report'].get_action(self, 'cus_center.print_dec_customs_invoice_template')
...
#     _inherit = 'work_sheet'
#
#     customs_declaration_ids = fields.One2many(comodel_name="cus_center.customs_order", inverse_name="work_sheet_id",
...
class GoodsWizard(models.TransientModel):
    _name = 'cus_center.goods_wizard'
    _description = 'Customs Goods Wizard'

    cus_goods_tariff_id = fields.Many2one(comodel_name="basedata.cus_goods_tariff", string="cus goods Code TS",
                                          required=False, )
    ...
    destination_country_id = fields.Many2one(comodel_name="delegate_country", string="destination country",
                                             required=False, )
    duty_mode_id = fields.Many2one(comodel_name="basedata.cus_duty_mode", string="Duty Mode", required=False, )
    goods_classification_id = fields.Many2one(comodel_name="cus_center.goods_classify",
                                              string="Goods Classification", required=False, )  # 客户料号
